...
from aiogram.types import Message, ReplyKeyboardRemove, InputMediaPhoto, InputMedia
...
from keyboards.keyboards import main_kb, bulletin_board_kb
from keyboards.creating_an_ad_keyboard import creating_an_ad_1, end_creating_ad
...
async def ad_sending(message: Message, bot: Bot, state: FSMContext):
    user_data = await state.get_data()
    if type(user_data['sended_photos']) == str:
